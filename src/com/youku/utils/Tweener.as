/*
   ...
   - Fixed an issue with tweening mcColor and alpha together.
   5.5:
...
package com.youku.utils {
    ...
    public class Tweener {
        ...
        public function Tweener($target:Object, $duration:Number, $vars:Object) {
            if ($target == null) {
                return
            }
            ;
        ...
        public function initTweenVals():void {
            var ndl:Number = this.delay - ((getTimer() - this.initTime) / 1000); //new delay. We need this because reversed (Tweener.from() calls) need to maintain the delay in any sub-tweens (like for color or volume tweens) but normal Tweener.to() tweens should have no delay because this function gets called only when the begin!
            ...
            if (this.target is Array) {
                ...
                for (p in this.vars) {
                    if (p == "volume" && this.target is MovieClip) { //If we're trying to change the volume of a MovieClip, then set up a quasai proxy using an instance of a Tweener to control the volume.
                        _sound = this.target.soundTransform;
                        var volTween:Tweener = new Tweener(this, this.duration, {volumeProxy: this.vars[p], ease: easeOut, delay: ndl, overwrite: false, runBackwards: this.vars.runBackwards});
                        volTween.endTarget = this.target;
                    } else if (p.toLowerCase() == "mccolor" && this.target is DisplayObject) { //If we're trying to change the color of a DisplayObject, then set up a quasai proxy using an instance of a Tweener to control the color.
                        ...
                        var colorTween:Tweener = new Tweener(this, this.duration, {colorProxy: 1, delay: ndl, overwrite: false, ease: this.vars.ease, runBackwards: this.vars.runBackwards});
        ...
        public static function to($target:Object, $duration:Number, $vars:Object):Tweener {
            return new Tweener($target, $duration, $vars);
        }
        ...
        public static function from($target:Object, $duration:Number, $vars:Object):Tweener {
            $vars.runBackwards = true;
            return new Tweener($target, $duration, $vars);
        }
        
        public static function delayedCall($delay:Number, $onComplete:Function, $onCompleteParams:Array = null):Tweener {
            return new Tweener($onComplete, 0, {delay: $delay, onComplete: $onComplete, onCompleteParams: $onCompleteParams, overwrite: false}); //NOTE / TO-DO: There may be a bug in the Dictionary class that causes it not to handle references to objects correctly! (I haven't verified this yet)
        }
        
        public static function removeTween($t:Tweener = null):void {
